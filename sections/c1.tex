...
main2 = (\x. 5) 10
\end{lstlisting}
\noindent Rather than simply reducing `\sflinline{f 10}' to 5 in one step, it would be useful to show the substitution to help users to understand why `\sflinline{f 10}' becomes `\sflinline{5}' here. This would involve substituting the variable \sflinline{f} for the function \sflinline{(\x. 5)}. We have two types of progress: reduction and substitution. In this section, we shall broaden the term `redex' to include both. 

\paragraph{Representing and Applying Redex-Contraction Pairs}
We can define a structure representing progress for our \ac{AST}: a pair where the first element is the node in the tree to be replaced, and the second element is an entirely new tree to replace it with. 
\begin{lstlisting}[language=Rust_boxed]
type RCPair = (usize, AST)
\end{lstlisting}
\noindent To do the substitution, we combine the two \ac{AST}s by concatenating their two lists of nodes, and replace all references to the original node to the root of the replacement \ac{AST}. 
...
Below is a summary of my client's thoughts about various aspects of the proof of concept system and potential future iterations
