...
\def\CompactJudgments{0}
  ...
  \end{mathpar}
  \caption{Algorithmic typing. The rules with \colorbox{myTcRuleColour}{highlighted} names are my additions, the rest are unchanged from \cite{completebidir}. Note that the checking rules $\MyTCRule{\Intcheckrulename}$, $\MyTCRule{\Boolcheckrulename}$, $\MyTCRule{\Paircheckrulename}$ are not actually necessary, as they could be caught by the $\Sub$ rule. They are included as they remove the unnecessary steps that using the $\Sub$ rule in this manner creates, speeding up/simplifying the algorithm}
