...
\end{alignat*}
A term is said to be in normal form if it cannot be $\beta$-reduced. A term that can be beta reduced can be said to be a \textit{redex} - reducible expression. The resulting term from reducing a redex is often called a contraction.

\subsection{Evaluation Strategies and Values}
We often have a term where we have multiple options for $\beta$-reduction. In this section, we will briefly discuss three different strategies that inform which option is selected when evaluation: call-by-value (a.k.a. strict), call-by-name and call-by-need (a.k.a. lazy). I have used the same examples as Pierce \cite{pierce2002types}. For instance, the closed term 
\[(\lam{x} x)\,((\lam{x} x)\,(\lam{z} (\lam{x}. x) z))\] has three redexes ($id$ is shorthand for $\lam{x} x$):
\[\underline{id\,(id\,(\lam{z} (\lam{x}. x) z))}\]
\[id\,\underline{(id\,(\lam{z} (\lam{x}. x) z))}\]
\[id\,(id\,(\lam{z} \underline{(\lam{x}. x) z}))\]

\noindent Most languages use the call-by-value evaluation strategy, where `only the outermost redexes are reduced and a redex is reduced only when its right-hand side has already been reduced to a value' \cite{pierce2002types}. This reduction strategy is also known as `strict'. Our chosen redex here would be number 2. 

The call-by-name strategy is less restrictive. `The leftomost outermost redex is always reduced first \ldots and allows no reductions inside abstractions'. Our reduction sequence would look like this:
...
It would be useful for us to be able to assert that a term eventually evaluates to one of these $Int$s. The \lcalc\ terms that evaluate to a value in the set of $Int$s can all be said to have `type' $Int$. More generally: 
\begin{quote}
...
The \lcalc\ that we have discussed so far is untyped. This means that any $\lambda$ term can be applied to any other term. 

\noindent When a term can no longer be $\beta$-reduced (i.e. no further reductions are possible), we say it is a value. This is why $\beta$-reduction is often referred to as \textit{evaluation} — it is the process by which terms are reduced to their final, fully evaluated form (i.e. values).
