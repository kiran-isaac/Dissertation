...
\noindent This definition of lists is as example of a polymorphic data type. It also implicitly defines two polymorphic constructors, \verb|[]| (Nil) which has type $\forall a. [a]$, and \verb|:| (Cons) which has a type $\forall a. a \fto [a] \fto [a]$. 
...
\label{c2:next_ui}
At this phase of the project, the current version of the web UI is a proof of concept. See \ref{fig:screenshot_phase_1_end} for the current state. This UI 

% I completely redesigned the UI based on the clients' feedback, as well as based on other requirements identified during the \hyperref[sec:c1_autoethnography]{autoethnographic} phase of the project. 



See \ref{fig:screenshot_figma1} and \ref{fig:screenshot_figma2} for screenshots of the new design. 
...
This design was meant to be a work in progress, but it looks quite similar to the final release of the product (see screenshots \ref{fig:screenshot_final_dark}, \ref{fig:screenshot_final_light}). 
Before implementing this design, I discussed this design with the Advanced Focus Group (see \ref{ref:afg_figma}) and they were much more positive about this UI than the existing one. 
% \todo{Discuss revert}\\
...
% \noindent This definition of `\sflinline{if}' contributed to problem of expressions becoming overly large during 
...
\begin{itemize}
    ...
    \item The program is saved in the browsers `localStorage' as it is edited to allow it to persist between website visits. 
...
\noindent The answers to these questions were mostly complimentary, but some useful information could be extracted:
\begin{itemize}
...
\noindent The visual glitches were also identified and fixed in phase 3. 
...
\begin{itemize}
    ...
    \item The third year optional Types and Lambda Calculus unit COMS30040 where they learnt about (among other things): \cite{COMS30040_TLC} 
    \begin{itemize}
...
\noindent These people I selected for this focus group were the closest to `subject experts' that I could find while still being undergraduate students. 
I started this focus group by briefly explaining the project. Figure~\ref{fig:screenshot_c2_end} shows how the system looked at this stage of the project. We also discussed the design of the next UI iteration (see~\ref{c2:next_ui}). 
...
\begin{quotation}
\noindent `The issue I was having is just the fact that there is a function in the prelude which has the same name as some syntactic sugar that is a parser construct'~[Participant~2,~42.55]
...
\paragraph{Negatives/Potential Improvements:}
\begin{itemize}
    \item `Syntax highlighting would obviously help' [Participant~1,~21:45]
    ...
    `You could put a number next to the reduction and say, you know, this is four steps. And then \ldots make a drop-down. Yeah. So if someone wants to see what steps are going on inside there, then they could see' [Participant~4,~08:57]
...
\begin{itemize}
    ...
    `It's easier to have everything on screen, and it's more akin to what people may have experienced \ldots\ Its like compiler explorer'. [Participant~3,~52:37] 
...
By the end of the project, I implemented the new UI along with syntax highlighting, and many other clarifying features. However, I unfortunately did not have time to work on grouping related reductions together or highlighting in source code when a progress option is hovered over what it would change, or improving the help menu. See the `future work' section \ref{conc:future_work} for more detail. 